...
function show_assistant_says_incremental_recursively(
  assistant_says_what_list,
  i
) {
  stop_assistant_istyping_temp();
  ...
  if (i < assistant_says_what_list.length - 1) {
    ...
    setTimeout(function () {
      show_assistant_says_incremental_recursively(
        assistant_says_what_list,
        i + 1
      );
...
function get_assistant_message(content) {
  ...
  return $("<div>")
    .addClass("d-flex flex-row justify-content-start mb-4")
    .append(
      $("<i>")
        .addClass("bi bi-emoji-sunglasses")
        .attr("style", "font-size: 2rem;"),
      $("<div>")
        .addClass("p-3 ms-3 border border-secondary")
        .attr("style", "border-radius: 15px;")
        .append($("<p>").addClass("small mb-0").html(content))
    );
}

function get_assistant_istyping_message() {
  return $("<div>")
    .addClass("temporary d-flex flex-row justify-content-start mb-4")
    .append(
      $("<i>")
        .addClass("bi bi-emoji-sunglasses")
        .attr("style", "font-size: 2rem;"),
      $("<div>")
        ...
        .append(
          $("<p>")
            .addClass("small mb-0")
            .append(
              $("<i>").addClass("bi bi-chat aaa").attr("id", "istyping_icon")
            )
        )
    );
}
...
function animate_istyping() {
  let current_icon_index = istyping_icons.indexOf(
    $("#istyping_icon").attr("class")
  );
...
function get_user_message(content) {
  return $("<div>")
    .addClass("d-flex flex-row justify-content-end mb-4")
    .append(
      $("<div>")
        .addClass("p-3 me-3 border border-secondary")
        .attr("style", "border-radius: 15px;")
        .append($("<p>").addClass("small mb-0").text(content)),
      $("<i>")
        .addClass("bi bi-person-bounding-box")
        .attr("style", "font-size: 2rem;")
    );
}
